...
个人设置页包含外观主题面板，提供九个预设，每个都忠实移植一套 Ant Design
官方主题：默认、暗黑、类 MUI、类 shadcn、卡通、插画、类 Bootstrap 拟物化、
玻璃和极客。每个预设都是 `src/theme/antd/` 下的一个 `antd-style` hook，返回完整
`ConfigProviderProps`——设计令牌、组件级覆盖，以及组件级 `classNames` 特效
（如极客的霓虹描边、MUI 的大写字母 + 点击涟漪按钮、Bootstrap 的渐变按钮、
插画的硬阴影）。只有玻璃预设保留壁纸/透明/毛玻璃材质（`liquid-glass.css`，已
作用域到玻璃预设），其余预设完全由各自的官方主题渲染。详见
`docs/theme-system.md`。

用户还可自定义核心令牌（主色/背景/表面/文字/边框颜色、圆角、密度和亮/暗/跟随
系统模式）。官方配色为默认值，仅覆盖用户实际改动的令牌。令牌驱动的预设会反映
全部自定义；少数深度定制预设（MUI、shadcn）在特效类中硬编码了按钮配色/圆角以
还原官方观感，故这些具体元素保持官方样式。预设与令牌通过 `/v1/auth/me/theme`
保存，并在下次登录时从 `/v1/auth/me` 自动恢复。管理员可在“系统设置 → 外观”中
设置全局默认预设、色彩模式和核心令牌 JSON，配置保存至 `system_settings`，只作用
于尚未保存个人主题偏好的用户。系统默认主题也可在同一面板中恢复为内置默认配置。
...
`Corporate LDAP (LDAP)` 或 `Local (LOCAL)`。管理员可以直接从来源标签删除
多来源用户的单个认证来源；后端会拒绝删除用户最后一个来源。管理员可以重置
本地用户密码；后端生成随机密码，前端只在弹窗中展示一次并支持复制。LDAP
用户密码由 LDAP 管理，不会在用户管理页重置。
